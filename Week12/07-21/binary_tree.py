...
def bst_traverse(node, fn, level=0):
    if node is None:
        return
    fn(node, level)
    bst_traverse(node.left, fn, level + 1)
    bst_traverse(node.right, fn, level + 1)
...
def printit(node, level):
    print "%s%d" % (" " * level, node.data)

bst_traverse(root_node, printit)

# result = bst_lookup(root_node, 29)
# print "Search result %r" % result
